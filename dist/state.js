...
export class StateClient {
    ...
    async get(path, opts) {
        ...
        // Build URL
        const url = new URL(`/v1/projects/${this.projectId}/state`, this.endpoint);
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new ApiError(`Failed to get state: ${text || response.statusText}`, response.status, text);
            }
    ...
    subscribe(callback, opts) {
        return createSubscription(this.endpoint, this.projectId, this.apiKey, callback, opts);
    }
    /**
    ...
    select(value, path) {
        ...
        for (const segment of segments) {
            ...
            // Cannot traverse further
                ...
                throw new PathError(`Cannot traverse non-object/array at segment: ${segment}`, path.toString());
            }
        }
        return current;
    }
