...
const getWebSocketImpl = () => {
    ...
    try {
        ...
        throw new WebSocketError("WebSocket not available. In Node.js, install 'ws' package: npm install ws");
    }
};
/**
...
export class StateSubscriptionClient {
    ...
    connect() {
        ...
        try {
            ...
            throw new WebSocketError(`Failed to connect to WebSocket: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
    ...
    scheduleReconnect() {
        ...
        const delay = Math.min(1000 * Math.pow(2, this.reconnectAttempt), this.opts.reconnectMaxDelayMs);
 ...
 */
export function createSubscriptionUrl(endpoint, projectId, apiKey) {
    // Convert HTTP(S) endpoint to WS(S)
    const wsEndpoint = endpoint.replace(/^http/, "ws");
    // Build URL with api_key in query param (for browser compatibility)
    const url = new URL(`/v1/projects/${projectId}/state/subscribe`, wsEndpoint);
 ...
 */
export function createSubscription(endpoint, projectId, apiKey, callback, opts) {
